...
class NumericDataImputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.medians_ = X.groupby(self.group_column_).median()
        return self
    ...
    def transform(self, X):
        ...
        for name, group in X.groupby(self.group_column_):
            for column in group.select_dtypes(include=['float']).columns:
                if column in self.medians_.columns:
                    ...
                    X_transformed.loc[X_transformed[self.group_column_] == name, column] = group[column].fillna(median_value)
